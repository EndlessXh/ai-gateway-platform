...
func TestDeriveCandidatesExplicitlySkipsFreeTier(t *testing.T) {
	freeModel := openRouterModel{
		ID:            "acme/example:free",
		Name:          "Acme: Example Free",
		Description:   "Synthetic free-tier fixture.",
		ContextLength: 8192,
	}
	freeModel.Architecture.InputModalities = []string{"text"}
	freeModel.Architecture.OutputModalities = []string{"text"}
	freeModel.Pricing.Prompt = "0"
	freeModel.Pricing.Completion = "0"

	candidates, skippedAlias, skippedBatch, skippedFree, skippedUnparsable := deriveCandidates([]openRouterModel{freeModel})

	assert.Empty(t, candidates)
	assert.Zero(t, skippedAlias)
	assert.Zero(t, skippedBatch)
	assert.Equal(t, 1, skippedFree)
	assert.Empty(t, skippedUnparsable, "a :free model is a product-policy skip, not an invalid identifier")
}
