...
func sessionView(session *model.UserSession, current bool) LoginSessionView {
	return LoginSessionView{
		...
		IP:           summarizeSessionIP(session.IP),
...
func summarizeSessionIP(value string) string {
	...
	for index := range groups {
		groups[index] = uint16(ipv6[index*2])<<8 | uint16(ipv6[index*2+1])
	}
	return fmt.Sprintf("%x:%x:%x:%x::", groups[0], groups[1], groups[2], groups[3])
}
