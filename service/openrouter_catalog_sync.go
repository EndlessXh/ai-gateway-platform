...
// and `:free` variants, then converts the remainder into catalogCandidate
// values. OpenRouter's free tier has strict rate limits and can be withdrawn at
// any time, so it is not suitable supply for a paid product. Two OpenRouter
// models that would strip to the same public_model_id (rare, but not impossible
// across vendors) both keep their full vendor-prefixed id instead — resolved
// with a pre-flight pass over the whole batch before any candidate is finalized,
