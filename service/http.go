...
func ShouldCopyUpstreamHeader(c *gin.Context, k string, v []string) bool {
	...
	if c != nil && c.GetInt("channel_type") == constant.ChannelTypeOpenRouter && isOpenRouterPrivateResponseHeader(k) {
		return false
	}
	return true
}

func isOpenRouterPrivateResponseHeader(name string) bool {
	lower := strings.ToLower(strings.TrimSpace(name))
	if strings.HasPrefix(lower, "x-openrouter-") {
		return true
	}
	switch lower {
	case "x-generation-id", "x-request-id", "x-upstream-request-id", "server", "via", "cf-ray", "cf-cache-status", "x-powered-by":
		return true
	default:
		return false
	}
