...
func TestSummarizeSessionIPProtectsPreciseAddress(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string
	}{
		{name: "ipv4", input: "203.0.113.42", want: "203.0.113.x"},
		{name: "ipv6", input: "2001:db8:abcd:12:3456:789a:bcde:f012", want: "2001:db8:abcd:12::"},
		{name: "compressed ipv6 does not expose host groups", input: "2001:db8::f012", want: "2001:db8:0:0::"},
		{name: "empty", input: "", want: ""},
		{name: "unparseable", input: "private-network", want: "hidden"},
		{name: "invalid dotted address", input: "999.999.999.999", want: "hidden"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assert.Equal(t, test.want, summarizeSessionIP(test.input))
		})
	}
}
