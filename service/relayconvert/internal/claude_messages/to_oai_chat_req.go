...
func ClaudeMessagesRequestToOpenAIChat(claudeRequest dto.ClaudeRequest, info *relaycommon.RelayInfo) (*dto.GeneralOpenAIRequest, error) {
	...
	if claudeRequest.ToolChoice != nil {
		...
		case "auto", "none":
			openAIRequest.ToolChoice = toolChoice.Type
		case "any":
		...
		if toolChoice.Type != "none" && toolChoice.DisableParallelToolUse {
	...
	for _, claudeMessage := range claudeRequest.Messages {
		...
		if claudeMessage.IsStringContent() {
			...
			reasoningDetails := make([]openRouterReasoningDetail, 0)
			...
			for _, mediaMsg := range content {
				...
				case "thinking":
					if isOpenRouter && mediaMsg.Thinking != nil {
						reasoningDetails = append(reasoningDetails, openRouterReasoningDetail{
							Type: "reasoning.text", Text: *mediaMsg.Thinking, Signature: mediaMsg.Signature,
						})
					}
				case "redacted_thinking":
					if isOpenRouter && mediaMsg.Data != "" {
						reasoningDetails = append(reasoningDetails, openRouterReasoningDetail{
							Type: "reasoning.encrypted", Data: mediaMsg.Data,
						})
					}
				}
			}
			...
			if len(reasoningDetails) > 0 {
				encoded, err := common.Marshal(reasoningDetails)
				if err != nil {
					return nil, fmt.Errorf("failed to marshal reasoning details: %w", err)
				}
				openAIMessage.ReasoningDetails = encoded
			}
		}
		if len(openAIMessage.ParseContent()) > 0 || len(openAIMessage.ToolCalls) > 0 || len(openAIMessage.ReasoningDetails) > 0 {
			openAIMessages = append(openAIMessages, openAIMessage)
		}
	}
