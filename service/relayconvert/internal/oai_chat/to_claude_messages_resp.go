...
func StreamResponseOpenAI2Claude(openAIResponse *dto.ChatCompletionsStreamResponse, info *relaycommon.RelayInfo) []*dto.ClaudeResponse {
	...
	emitReasoningDetails := func(raw []byte) bool {
		if len(raw) == 0 {
			return false
		}
		var details []openAIReasoningDetail
		if common.Unmarshal(raw, &details) != nil || len(details) == 0 {
			return false
		}
		emitted := false
		for _, detail := range details {
			...
			case "reasoning.text", "reasoning.summary":
				...
				if info.ClaudeConvertInfo.LastMessagesType != relaycommon.LastMessageTypeThinking {
					...
					claudeResponses = append(claudeResponses, &dto.ClaudeResponse{
						...
						ContentBlock: &dto.ClaudeMediaMessage{Type: "thinking", Thinking: common.GetPointer("")},
					})
				...
				if text != "" {
					claudeResponses = append(claudeResponses, &dto.ClaudeResponse{
						Index: &idx, Type: "content_block_delta",
						Delta: &dto.ClaudeMediaMessage{Type: "thinking_delta", Thinking: common.GetPointer(text)},
					})
				}
				if detail.Signature != "" {
					claudeResponses = append(claudeResponses, &dto.ClaudeResponse{
						Index: &idx, Type: "content_block_delta",
						Delta: &dto.ClaudeMediaMessage{Type: "signature_delta", Signature: detail.Signature},
					})
				}
				emitted = true
			case "reasoning.encrypted":
				...
				claudeResponses = append(claudeResponses,
					&dto.ClaudeResponse{
						Index: &idx, Type: "content_block_start",
						ContentBlock: &dto.ClaudeMediaMessage{Type: "redacted_thinking", Data: detail.Data},
					},
					generateStopBlock(idx),
				)
				info.ClaudeConvertInfo.Index++
				info.ClaudeConvertInfo.LastMessagesType = relaycommon.LastMessageTypeNone
				emitted = true
	...
	if info.SendResponseCount == 1 {
		...
		if len(openAIResponse.Choices) > 0 {
			detailsEmitted := emitReasoningDetails(openAIResponse.Choices[0].Delta.ReasoningDetails)
			reasoning := openAIResponse.Choices[0].Delta.GetReasoningContent()
			content := openAIResponse.Choices[0].Delta.GetContentString()

			if detailsEmitted {
				// reasoning_details is the lossless OpenRouter surface; do not
				// duplicate the legacy reasoning field when both are present.
			} else if reasoning != "" {
				if info.ClaudeConvertInfo.LastMessagesType != relaycommon.LastMessageTypeThinking {
					stopOpenBlocksAndAdvance()
				}
	...
	if len(openAIResponse.Choices) == 0 {
		...
		if len(chosenChoice.Delta.ToolCalls) > 0 {
			...
			detailsEmitted := emitReasoningDetails(chosenChoice.Delta.ReasoningDetails)
			reasoning := chosenChoice.Delta.GetReasoningContent()
			textContent := chosenChoice.Delta.GetContentString()
			if detailsEmitted {
				isEmpty = true
			} else if reasoning != "" || textContent != "" {
				if reasoning != "" {
					if info.ClaudeConvertInfo.LastMessagesType != relaycommon.LastMessageTypeThinking {
						stopOpenBlocksAndAdvance()
						idx := info.ClaudeConvertInfo.Index
...
func ResponseOpenAI2Claude(openAIResponse *dto.OpenAITextResponse, info *relaycommon.RelayInfo) *dto.ClaudeResponse {
	...
	for _, choice := range openAIResponse.Choices {
		...
		thinkingBlocks := openAIMessageThinkingBlocks(choice.Message)
		contents = append(contents, thinkingBlocks...)
		if textContent != "" || (len(toolCalls) == 0 && len(thinkingBlocks) == 0) {
...
func openAIMessageThinkingBlocks(message dto.Message) []dto.ClaudeMediaMessage {
	blocks := make([]dto.ClaudeMediaMessage, 0)
	var details []openAIReasoningDetail
	if len(message.ReasoningDetails) > 0 && common.Unmarshal(message.ReasoningDetails, &details) == nil {
		for _, detail := range details {
			switch detail.Type {
			case "reasoning.text":
				if detail.Text != "" {
					blocks = append(blocks, dto.ClaudeMediaMessage{
						Type: "thinking", Thinking: common.GetPointer(detail.Text), Signature: detail.Signature,
					})
				}
			case "reasoning.summary":
				if detail.Summary != "" {
					blocks = append(blocks, dto.ClaudeMediaMessage{
						Type: "thinking", Thinking: common.GetPointer(detail.Summary), Signature: detail.Signature,
					})
				}
			case "reasoning.encrypted":
				if detail.Data != "" {
					blocks = append(blocks, dto.ClaudeMediaMessage{Type: "redacted_thinking", Data: detail.Data})
				}
			}
		}
	}
	if len(blocks) == 0 {
		if thinking := message.GetReasoningContent(); thinking != "" {
			blocks = append(blocks, dto.ClaudeMediaMessage{
				Type: "thinking", Thinking: common.GetPointer(thinking),
			})
		}
	}
	return blocks
