...
func TestResponseOpenAI2ClaudePreservesOpenRouterThinking(t *testing.T) {
	reasoning := "brief reasoning"
	resp := ResponseOpenAI2Claude(&dto.OpenAITextResponse{
		Id: "chatcmpl_local", Model: "claude-opus-4.6",
		Choices: []dto.OpenAITextResponseChoice{{
			Message: dto.Message{
				Role: "assistant", Content: "2", Reasoning: &reasoning,
				ReasoningDetails: []byte(`[{"type":"reasoning.text","text":"brief reasoning","signature":"signed"}]`),
			},
			FinishReason: "stop",
		}},
	}, nil)
	...
	assert.Equal(t, "signed", resp.Content[0].Signature)
	assert.Equal(t, "text", resp.Content[1].Type)
	assert.Equal(t, "2", resp.Content[1].GetText())
}

func TestResponseOpenAI2ClaudePreservesAllReasoningDetailBlocks(t *testing.T) {
	resp := ResponseOpenAI2Claude(&dto.OpenAITextResponse{
		Id: "chatcmpl_local", Model: "claude-opus-4.6",
		Choices: []dto.OpenAITextResponseChoice{{
			Message: dto.Message{
	...
	assert.Equal(t, "thinking", resp.Content[0].Type)
	assert.Equal(t, "first", *resp.Content[0].Thinking)
	assert.Equal(t, "signed", resp.Content[0].Signature)
	assert.Equal(t, "redacted_thinking", resp.Content[1].Type)
	encoded, err := common.Marshal(resp.Content[1])
	require.NoError(t, err)
	assert.JSONEq(t, `{"type":"redacted_thinking","data":"encrypted-payload"}`, string(encoded))
	assert.Equal(t, "thinking", resp.Content[2].Type)
	assert.Equal(t, "second", *resp.Content[2].Thinking)
	assert.Equal(t, "text", resp.Content[3].Type)
}
...
func TestStreamResponseOpenAI2ClaudePreservesReasoningDetails(t *testing.T) {
	...
	responses := StreamResponseOpenAI2Claude(&dto.ChatCompletionsStreamResponse{
		Id: "chatcmpl_1", Model: "claude-opus-4.6",
		Choices: []dto.ChatCompletionsStreamResponseChoice{{
			Delta: dto.ChatCompletionsStreamResponseChoiceDelta{
				ReasoningDetails: []byte(`[{"type":"reasoning.text","text":"thinking","signature":null,"index":0}]`),
			},
		}},
	}, info)
	...
	assert.Equal(t, "thinking", *responses[2].Delta.Thinking)

	info.SendResponseCount = 2
	responses = StreamResponseOpenAI2Claude(&dto.ChatCompletionsStreamResponse{
		Choices: []dto.ChatCompletionsStreamResponseChoice{{
			Delta: dto.ChatCompletionsStreamResponseChoiceDelta{
				ReasoningDetails: []byte(`[{"type":"reasoning.text","signature":"signed","index":0}]`),
			},
		}},
	}, info)
	require.Len(t, responses, 1)
	assert.Equal(t, "signature_delta", responses[0].Delta.Type)
	assert.Equal(t, "signed", responses[0].Delta.Signature)

	info.SendResponseCount = 3
	responses = StreamResponseOpenAI2Claude(&dto.ChatCompletionsStreamResponse{
		Choices: []dto.ChatCompletionsStreamResponseChoice{{
			Delta: dto.ChatCompletionsStreamResponseChoiceDelta{
				ReasoningDetails: []byte(`[{"type":"reasoning.encrypted","data":"encrypted-payload","index":1}]`),
			},
		}},
	}, info)
	require.Len(t, responses, 3)
	assert.Equal(t, "content_block_stop", responses[0].Type)
	assert.Equal(t, "redacted_thinking", responses[1].ContentBlock.Type)
	assert.Equal(t, "encrypted-payload", responses[1].ContentBlock.Data)
