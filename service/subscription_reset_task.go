...
func runSubscriptionQuotaResetOnce() {
	...
	totalReset := 0
	totalExpired := 0
	if strings.EqualFold(strings.TrimSpace(os.Getenv("PRICING_STATUS")), "published") && operation_setting.IsPaymentComplianceConfirmed() {
		if _, err := model.ProcessDuePlatformSubscriptionRenewals(subscriptionResetBatchSize); err != nil {
			logger.LogWarn(ctx, fmt.Sprintf("platform subscription renewal task failed: %v", err))
		}
	}
	for {
	...
	if _, err := model.ReconcilePlatformSubscriptionLifecycles(subscriptionResetBatchSize); err != nil {
		logger.LogWarn(ctx, fmt.Sprintf("platform subscription lifecycle reconciliation failed: %v", err))
	}
	for {
