...
func GetAndValidateRequest(c *gin.Context, format types.RelayFormat) (request dto.Request, err error) {
	if err := rejectOpenRouterUserRoutingControls(c, format); err != nil {
		return nil, err
	}
...
var openRouterForbiddenRequestFields = map[string]struct{}{
	"provider": {}, "route": {}, "models": {}, "fallbacks": {},
	"plugins": {}, "transforms": {}, "debug": {}, "trace": {},
	"metadata": {}, "session_id": {},
}

var openRouterForbiddenRequestHeaders = []string{
...
func rejectOpenRouterUserRoutingControls(c *gin.Context, format types.RelayFormat) error {
	if c == nil || c.Request == nil || c.GetInt("channel_type") != channelconstant.ChannelTypeOpenRouter {
		return nil
	}
	for _, header := range openRouterForbiddenRequestHeaders {
		if strings.TrimSpace(c.Request.Header.Get(header)) != "" {
			return errors.New("routing control header is not allowed")
		}
	}
	...
	for field := range body {
		// Anthropic Messages metadata is a documented client field used by
		// Claude Code. ClaudeMessagesRequestToOpenAIChat intentionally does not
		// forward it, so accepting it here cannot influence OpenRouter routing or
		// expose the client value upstream.
		if strings.EqualFold(field, "metadata") && format == types.RelayFormatClaude {
			continue
		}
		if _, forbidden := openRouterForbiddenRequestFields[strings.ToLower(field)]; forbidden {
			return fmt.Errorf("routing control field %q is not allowed", field)
		}
	}
	return nil
}
