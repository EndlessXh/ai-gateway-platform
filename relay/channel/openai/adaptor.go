...
func (a *Adaptor) SetupRequestHeader(c *gin.Context, header *http.Header, info *relaycommon.RelayInfo) error {
	...
	if info.ChannelType == constant.ChannelTypeOpenRouter {
		// OpenRouter response metadata is opt-in. Keep it explicitly disabled and
		// do not send application identity headers that reveal the upstream route.
		header.Del("HTTP-Referer")
		header.Del("X-Title")
		header.Del("X-OpenRouter-Title")
		header.Set("X-OpenRouter-Metadata", "disabled")
	}
	return nil
}
