...
func enforceOpenRouterPrivateRequestHeaders(req *http.Request, info *common.RelayInfo) {
	if req == nil || info == nil || info.ChannelType != channelconstant.ChannelTypeOpenRouter {
		return
	}
	req.Header.Del("HTTP-Referer")
	req.Header.Del("X-Title")
	req.Header.Del("X-OpenRouter-Title")
	req.Header.Del("X-OpenRouter-Experimental-Metadata")
	req.Header.Del("X-Session-Id")
	req.Header.Set("X-OpenRouter-Metadata", "disabled")
}
...
func captureOpenRouterRetryAfter(c *gin.Context, info *common.RelayInfo, resp *http.Response) {
	if c == nil || info == nil || info.ChannelMeta == nil || resp == nil || info.ChannelType != channelconstant.ChannelTypeOpenRouter {
		return
	}
	if retryAfter := resp.Header.Get("Retry-After"); retryAfter != "" {
		c.Set(service.UpstreamRetryAfterContextKey, retryAfter)
	}
}
