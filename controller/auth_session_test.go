...
func TestRevokeAllLoginSessionsScopesToAuthenticatedUser(t *testing.T) {
	previousDB := model.DB
	previousRedis := common.RedisEnabled
	previousSecret := common.SessionSecret
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	...
	model.DB = db
	common.RedisEnabled = false
	common.SessionSecret = "revoke-all-sessions-test-secret"
	t.Cleanup(func() {
		model.DB = previousDB
		common.RedisEnabled = previousRedis
		common.SessionSecret = previousSecret
	...
	users := []model.User{
		{Username: "revoke-all-owner", Password: "unused", Role: common.RoleCommonUser, Status: common.UserStatusEnabled, Group: "default", AuthVersion: 1, AffCode: "revoke-all-owner"},
		{Username: "revoke-all-other", Password: "unused", Role: common.RoleCommonUser, Status: common.UserStatusEnabled, Group: "default", AuthVersion: 1, AffCode: "revoke-all-other"},
	}
	require.NoError(t, db.Create(&users).Error)
	ownerSessionA, err := service.CreateLoginSession(users[0].Id, "password", "127.0.0.1", "owner-a")
	require.NoError(t, err)
	ownerSessionB, err := service.CreateLoginSession(users[0].Id, "password", "127.0.0.2", "owner-b")
	require.NoError(t, err)
	otherSession, err := service.CreateLoginSession(users[1].Id, "password", "127.0.0.3", "other")
	require.NoError(t, err)

	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/user/sessions/revoke-all", nil)
	c.Request.AddCookie(&http.Cookie{Name: service.RefreshCookieName, Value: ownerSessionA.RefreshToken})
	c.Set("id", users[0].Id)
	c.Set("session_id", ownerSessionA.Session.SID)
	c.Set("auth_version", int64(1))
	c.Set("session_version", int64(1))

	RevokeAllLoginSessions(c)
	...
	assert.Equal(t, int64(2), response.Data.RevokedCount)
	assert.Contains(t, recorder.Header().Get("Set-Cookie"), service.RefreshCookieName+"=;")

	for _, sid := range []string{ownerSessionA.Session.SID, ownerSessionB.Session.SID} {
		stored, err := model.GetUserSessionBySID(sid)
		require.NoError(t, err)
		assert.Equal(t, model.UserSessionStatusRevoked, stored.Status)
	}
	storedOther, err := model.GetUserSessionBySID(otherSession.Session.SID)
	require.NoError(t, err)
	assert.Equal(t, model.UserSessionStatusActive, storedOther.Status)
}

func TestRevokeAllLoginSessionsRequiresBrowserSession(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/user/sessions/revoke-all", nil)
