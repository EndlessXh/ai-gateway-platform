...
func TestBuildOpenRouterModelUsesPlatformOwner(t *testing.T) {
	modelItem := buildOpenAIModel("claude-opus-4.6", map[string]string{"claude-opus-4.6": channelOwnerName(constant.ChannelTypeOpenRouter)})
	require.Equal(t, "claude-opus-4.6", modelItem.Id)
	require.Equal(t, "hyc-ai", modelItem.OwnedBy)
}
