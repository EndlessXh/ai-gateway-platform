...
func GetSubscriptionPlans(c *gin.Context) {
	...
	profiledPlanIds := model.DB.Model(&model.PlatformSubscriptionPlanProfile{}).Select("plan_id")
	if err := model.DB.Where("enabled = ? AND id NOT IN (?)", true, profiledPlanIds).Order("sort_order desc, id desc").Find(&plans).Error; err != nil {
...
func rejectPlatformPlanOnLegacyPath(c *gin.Context, planId int) bool {
	isPlatformPlan, err := model.IsPlatformSubscriptionPlan(planId)
	if err != nil {
		common.SysError("legacy subscription plan gate failed: " + err.Error())
		common.ApiErrorMsg(c, "subscription operation failed")
		return true
	}
	if isPlatformPlan {
		common.ApiErrorMsg(c, "product plans must use the platform subscription lifecycle API")
		return true
	}
	return false
}
...
func SubscriptionRequestBalancePay(c *gin.Context) {
	...
	if rejectPlatformPlanOnLegacyPath(c, req.PlanId) {
		return
	}
...
func AdminListSubscriptionPlans(c *gin.Context) {
	var plans []model.SubscriptionPlan
	profiledPlanIds := model.DB.Model(&model.PlatformSubscriptionPlanProfile{}).Select("plan_id")
	if err := model.DB.Where("id NOT IN (?)", profiledPlanIds).Order("sort_order desc, id desc").Find(&plans).Error; err != nil {
		common.ApiError(c, err)
		return
	}
	result := make([]SubscriptionPlanDTO, 0, len(plans))
...
func AdminUpdateSubscriptionPlan(c *gin.Context) {
	...
	if rejectPlatformPlanOnLegacyPath(c, id) {
		return
	}
	var req AdminUpsertSubscriptionPlanRequest
	if err := c.ShouldBindJSON(&req); err != nil {
