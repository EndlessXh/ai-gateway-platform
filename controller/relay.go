	...
	defer func() {
		if newAPIError != nil {
			...
			if c.GetInt("channel_type") == constant.ChannelTypeOpenRouter {
				newAPIError.SetMessage(service.OpenRouterPublicErrorMessage(newAPIError.StatusCode))
				if newAPIError.StatusCode == http.StatusTooManyRequests || newAPIError.StatusCode == http.StatusServiceUnavailable {
					if retryAfter := service.NormalizeRetryAfter(c.GetString(service.UpstreamRetryAfterContextKey)); retryAfter != "" {
						c.Header("Retry-After", retryAfter)
					}
				}
			}
			newAPIError.SetMessage(common.MessageWithRequestId(newAPIError.Error(), requestId))
			// A streaming adaptor may already have emitted a protocol-native terminal
			// error event. Appending a JSON HTTP error after SSE has started corrupts
			// the stream and makes clients report a misleading generic 500.
...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		if c.GetInt("channel_type") == constant.ChannelTypeOpenRouter {
			adminInfo["upstream_error"] = logContent
			logContent = fmt.Sprintf("status_code=%d, %s", err.StatusCode, service.OpenRouterPublicErrorMessage(err.StatusCode))
		}
