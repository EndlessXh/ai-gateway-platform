...
export function RankingsHero(props: RankingsHeroProps) {
  ...
  return (
    /*
      Left-aligned on purpose: the reference leaderboard is left-aligned, so
      this is deliberately NOT the centred masthead used by the pricing and
      catalogue pages.

      What was wrong was the type scale, not the alignment — this heading used
      a one-off `clamp(1.75rem,4vw,2.5rem)` that existed nowhere else, so the
      leaderboard title was visibly smaller than every other page title. It now
      uses the shared `type-page-title` and the shared eyebrow/lede treatment,
      which also aligns its left edge and vertical rhythm with the panels below.
    */
    <section className='space-y-6'>
      <div>
        ...
        <h1 className='type-page-title type-page-title-sm'>
          {t('Rankings')}
        </h1>
