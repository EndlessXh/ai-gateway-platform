...
export function ModelsSection(props: ModelsSectionProps) {
  ...
  const spec = useMemo(() => {
    ...
    return {
      ...
      stack: true,
      /*
        Cap the bar width. A band scale divides the plot area by the number of
        categories, so with the two days of history this deployment actually
        has, each bar became a slab roughly 30% of the chart wide and the panel
        read as a broken graphic rather than a sparse one. The cap only binds
        when categories are few; at 7, 30 or 365 points the natural width is
        already far below it, so denser periods are unaffected.

        This changes presentation only — no point is added, removed or
        interpolated.
      */
      barMaxWidth: 64,
