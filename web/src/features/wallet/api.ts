...
export async function getRecentWalletConsumption(
  startTimestamp: number
): Promise<ApiResponse<{ items: WalletUsageRecord[]; total: number }>> {
  const params = new URLSearchParams({
    p: '1',
    page_size: '5',
    type: '2',
...
export async function getWalletUsageSummary(
  ...
  const res = await api.get(`/api/log/self/stat?${params.toString()}`)
  return res.data
}
