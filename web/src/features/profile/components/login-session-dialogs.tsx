...
interface LoginSessionDialogsProps {
  ...
  onConfirmAllChange: (open: boolean) => void
  onRevoke: () => void
  onRevokeOthers: () => void
  onRevokeAll: () => void
}
...
export function LoginSessionDialogs({
  ...
  return (
    <>
      ...
      <ConfirmDialog
        open={confirmAll}
        onOpenChange={onConfirmAllChange}
        title={t('Sign out every session?')}
        desc={t(
          'Every device, including this one, will lose access immediately. You must sign in again.'
        )}
        confirmText={t('Sign out everywhere')}
        destructive
        isLoading={revokingAll}
        handleConfirm={onRevokeAll}
      />
