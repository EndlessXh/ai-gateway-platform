...
/*
  Both conditions are required. The mode check keeps the panels out of any
  production bundle even if the variable leaks into a release environment;
  Rsbuild exposes `PUBLIC_*` variables to the client, so `PUBLIC_DEVTOOLS=1`
  is what a developer opts in with locally.
*/
const DEVTOOLS_ENABLED =
  import.meta.env.MODE === 'development' &&
  import.meta.env.PUBLIC_DEVTOOLS === '1'
...
function RootComponent() {
  ...
  return (
    <ThemeCustomizationProvider>
      ...
      {/*
        Devtools are opt-in, not on-by-default in development.

        They were previously mounted for the whole of `MODE === 'development'`,
        which put the Router panel bottom-right and the React Query button
        bottom-left on every page. Since `rsbuild.config.ts` binds port 4173 for
        both `dev` and `preview`, anyone reviewing the product at the agreed
        acceptance URL saw two floating overlays and could not tell which build
        they were looking at. They were never in the production bundle — this
        removes them from visual review too.

        Turn them on with PUBLIC_DEVTOOLS=1 in a local shell.
      */}
      {DEVTOOLS_ENABLED && (
        <>
