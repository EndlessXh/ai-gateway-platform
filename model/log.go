...
func formatUserLogs(logs []*Log, startIdx int) {
	for i := range logs {
		...
		if otherMap != nil {
			...
			// [platform] Provider isolation: users buy platform model aliases and
			// must not be able to read back which upstream model served them.
			// Leaking it would both expose the routing topology and let the
			// published catalogue be contradicted by the logs. Admin log views do
			// not call formatUserLogs, so operators still see this for debugging.
			delete(otherMap, "upstream_model_name")
			// Older error records stored channel routing fields at the top level of
			// Other. Strip them as well as the typed Log fields for user views.
