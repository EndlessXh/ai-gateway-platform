...
func Redeem(key string, userId int) (quota int, err error) {
	...
	// Redeem updates quota in the transaction above. An authenticated user may
	// already have an older quota in Redis, so discard that snapshot before the
	// next relay request reads it.
	if err := invalidateUserCache(userId); err != nil {
		common.SysError("failed to invalidate redeemed user cache: " + err.Error())
	}
