...
func downgradeUserGroupForSubscriptionTx(tx *gorm.DB, sub *UserSubscription, now int64) (string, error) {
	...
	// Preserve a group changed by an administrator or another non-subscription
	// source after this subscription was applied.
	if upgradeGroup != "" && currentGroup != upgradeGroup {
		return "", nil
	}
	// When the ending subscription supplied the current group, fall back to the
	// strongest remaining active subscription before considering its original
...
// AdminDeleteUserSubscription is retained for API compatibility, but financial
// subscription records are immutable. Operators must invalidate them instead.
func AdminDeleteUserSubscription(userSubscriptionId int) (string, error) {
	if userSubscriptionId <= 0 {
		return "", errors.New("invalid userSubscriptionId")
	}
	return "", errors.New("subscription financial records are immutable; invalidate the subscription instead")
}
