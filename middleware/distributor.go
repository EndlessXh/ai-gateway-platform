...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if modelRequest.Model != "" {
			cataloged, enabled, catalogErr := service.IsPlatformModelAPIEnabled(modelRequest.Model)
			if catalogErr != nil {
				abortWithOpenAiMessage(c, http.StatusServiceUnavailable, "The model catalog is temporarily unavailable.")
				return
			}
			if cataloged && !enabled {
				abortWithOpenAiMessage(c, http.StatusForbidden, "The requested model is not enabled for API access.")
				return
			}
		}
		if ok {
