...
func TestTokenAuthRejectsConflictingMessagesCredentials(t *testing.T) {
	...
	request.Header.Set("Authorization", "Bearer authorization-credential")
	request.Header.Set("x-api-key", "different-credential")
	response := httptest.NewRecorder()

	router.ServeHTTP(response, request)

	assert.Equal(t, http.StatusUnauthorized, response.Code)
	assert.NotContains(t, response.Body.String(), "authorization-credential")
	assert.NotContains(t, response.Body.String(), "different-credential")
}
