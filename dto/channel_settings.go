...
type ChannelOtherSettings struct {
	...
	AllowOpenRouterDataRetention          bool                  `json:"allow_openrouter_data_retention,omitempty"` // 是否允许 OpenRouter 路由到不承诺零数据保留(ZDR)的服务商（仅 OpenRouter 渠道生效，默认禁止；某些模型在 OpenRouter 上当前没有满足 ZDR 的可用端点，开启后该渠道对这些模型的请求可能被上游保留）
